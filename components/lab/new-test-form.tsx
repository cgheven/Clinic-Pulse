...
import { recordTest } from '@/app/actions/lab'
...
// Client form
//
// Dropdown data (`initialTests` / `initialPaymentMethods`) is fetched on the
// server — same pattern as app/(dashboard)/opd/visits/new/page.tsx — so the
// form is usable the moment it paints.
// =============================================================================
...
export function NewTestForm({
  ...
  initialTests: CpLabTest[]
  initialPaymentMethods: Array<{ method: string; label: string }>
}) {
  const router = useRouter()
  const [form, setForm] = useState<FormState>(DEFAULT_FORM)
  const [errors, setErrors] = useState<FormErrors>({})
  const [tests] = useState<CpLabTest[]>(initialTests)
  const [paymentMethods] = useState<Array<{ method: string; label: string }>>(
    initialPaymentMethods
  )
