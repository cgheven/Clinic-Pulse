...
export const metadata: Metadata = {
  title: 'Payroll Report — ClinicPulse',
}
...
// getPayrollReport() call (and therefore a single DB round-trip) per request.
// =============================================================================

const loadReport = cache(getPayrollReport)
...
async function ReportPdf({ period }: { period: string }) {
  ...
  return (
    <PdfGenerator
      report={{ type: 'payroll', data: result.data }}
      fileName={`payroll-${period}`}
    />
  )
}
...
async function ReportContent({ period }: { period: string }) {
  ...
  return <PayrollTable data={result.data} />
}
  ...
  return (
      <div className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href={`/reports/payroll?month=${nextMonth}`}
            className="rounded-lg border border-border bg-card p-2 text-muted-foreground hover:border-primary/30 hover:text-foreground transition-colors"
