...
// getPartnerPayoutReport() call (and therefore a single DB round-trip) per request.
// =============================================================================

const loadReport = cache(getPartnerPayoutReport)
...
async function ReportPdf({ period }: { period: string }) {
  ...
  return (
    <PdfGenerator
      ...
      fileName={`partner-payouts-${period}`}
    />
  )
}
...
async function ReportContent({ period }: { period: string }) {
  ...
  return <PartnerPayoutTable data={result.data} />
}
