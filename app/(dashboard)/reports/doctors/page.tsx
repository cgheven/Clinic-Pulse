...
import { ChevronLeft, ChevronRight, Stethoscope, AlertTriangle } from 'lucide-react'
...
export const metadata: Metadata = {
  title: 'Doctor Earnings Report — ClinicPulse',
}
...
export default async function DoctorEarningsPage({ searchParams }: PageProps) {
  await requireAuth()
    ...
    <div className="space-y-0">
      ...
      <div className="space-y-6 p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Per-doctor earnings breakdown — salaried & commission models
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/reports/doctors?month=${prevMonth}`}
          ...
          <Link
            href={`/reports/doctors?month=${nextMonth}`}
            className="rounded-lg border border-border bg-card p-2 text-muted-foreground hover:border-primary/30 hover:text-foreground transition-colors"
